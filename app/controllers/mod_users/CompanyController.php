...
class CompanyController extends \BaseController {
    ...
    public function index() {
        ...
        if (\Auth::check()) {
            if ($check->is('administrator')) {
                return \View::make('mod_users.admin.company.index', $data);
            } elseif ($check->is('employee')) {
                
            }
        } else {
            return \View::make('mod_users.guest.company', $data);
        }
    }
    ...
    public function listall() {
        ...
        $link .= '<li><a href="javascript:;" rel="users/company/edit/{{$id}}" class="link_dialog" title="แก้ไขรายการ"><i class="fa fa-pencil-square-o"></i> แก้ไขรายการ</a></li>';
        $link .= '<li><a href="javascript:;" rel="users/company/delete/{{$id}}" class="link_dialog delete" title="ลบรายการ"><i class="fa fa-trash"></i> ลบรายการ</a></li>';
        $link .= '</ul>';
        $link .= '</div>';
    ...
    public function add() {
        if (!\Request::isMethod('post')) {
            return \View::make('mod_users.admin.company.add');
        } else {
            $rules = array(
                'title' => 'required',
                'address1' => 'required'
    ...
    public function edit($param) {
        if (!\Request::isMethod('post')) {
            ...
            return \View::make('mod_users.admin.company.edit', $data);
        } else {
            $rules = array(
                'title' => 'required',
                'address1' => 'required'
