...
class HswareController extends \BaseController {
    ...
    public function model_sub_listall($param) {
        ...
        $link .= '<a class="dropdown-toggle" data-toggle="dropdown" href="javascript:;"><span class="fa fa-pencil-square-o"></span ></a>';
        $link .= '<ul class="dropdown-menu" role="menu" aria-labelledby="dLabel">';
        $link .= '<li><a href="javascript:;" rel="mis/hsware/group/model/sub/edit/{{$id}}" class="link_dialog" title="แก้ไขรายการ"><i class="fa fa-pencil-square-o"></i> แก้ไขรายการ</a></li>';
    ...
    public function model_sub_edit($param) {
        if (!\Request::isMethod('post')) {
            $data = array(
                'item' => \HswareModel::find($param)
            );
            return \View::make('mod_mis.hsware.admin.model_sub_edit', $data);
        } else {
            $rules = array(
                'title' => 'required'
            );
